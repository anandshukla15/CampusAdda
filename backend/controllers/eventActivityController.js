...
const getEvent = async (eventId) => {
  const rows = await EventActivity.query(
  ...
  try {
    ...
    if (!canManageEvent(event, req.user)) {
      return res.status(403).json({
        error: "You can only manage activities for your own events"
      });
    }

    const activity = EventActivity.normalizeActivity(
      req.body,
      event_id,
      event.category
    );
    ...
    const validationError =
      EventActivity.validateActivity(activity);
...
exports.getActivitiesByEvent = async (req, res) => {
  try {
    ...
    const activities = await EventActivity.findByEventId(
      req.params.eventId
    );
...
exports.updateRegistrationStatus = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error: "registration_open is required"
      });
    }
